 ...
 * @LastEditors  : MCD
 * @LastEditTime : 2021-08-16 10:31:58
...
globalmem_write(struct file *filp, const char __user *buf, size_t size, loff_t *ppos)
{
    ...
    if(copy_from_user(dev->mem + p, buf, count))
        ...
        *ppos += count;
        ret = count;
        printk(KERN_ALERT , "written %u dytes(s) from %lu\n", count, p);
    }    
...
fail_malloc:
    unregiset_chrdev_region(devno, DEVICE_NUM);
    return ret;
}
...
globalmem_exit(void)
{
    ...
    unregist_chrdev_region(MKDEV(globalmem_major, 0), DEVICE_NUM);
}
