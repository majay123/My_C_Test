 ...
 * @LastEditors  : MCD
 * @LastEditTime : 2023-08-22 14:22:16
...
// {"pid":"fkxcslivaluonzdp","cids":["01d1dac506","01831cc03a"],"ver":"1.0.2"} //调光灯1 调光灯2
// {"pid":"fkxcslivaluonzdp","cids":["0183796f6a"],"ver":"1.0.2"} //调光灯3
// {"pid":"fkxcslivaluonzdp","cids":["01831ade6b"],"ver":"1.0.2"}//调光灯4
// {"pid":"fkxcslivaluonzdp","cids":["01d1efe2c9"],"ver":"1.0.2"}//调光灯5
// {"pid":"5zkpavp5cxonvvxl","cids":["b223f59481","b2838807f1"],"ver":"1.0.1"}
// {"pid":"lroj54nnitbqwfct","cids":["b2836ba70f"],"ver":"1.0.1"}

// 14 01 00 01 01
// 16 02 00 04 00 00 03 E8
// 17 02 00 04 00 00 01 F4
// 18 03 00 0C 30 30 30 30 30 30 30 30 30 33 65 38
// 6A 03 00 08 31 39 34 39 35 31 33 31

// 55 AA 00 0D 00 15
// 04 30 30 30 30
// 6B 03 00 0C 30 30 33 38 64 34 36 36 36 38 30 30 FC

// 55 AA 00 0D 00 15
// 04 30 30 30 30
// 6B 03 00 0C 46 46 46 46 46 46 46 46 46 46 46 46 FC
// 55 AA 00 0D 00 15 04 30 30 30 30 6B 03 00 0C 35 35 30 30 66 66 66 66 66 66 61 61 4F
// 55 aa 00 0c 00 15 04 30 30 30 30 6b 03 00 0c 46 46 46 46 46 46 46 46 46 46 46 46 a6

void leshi_sure_band(void)
{
    leshi_mesh_t test_cmd;
    // uint8_t *data = "{\"cid\":[\"b2838807f1\", \"b223f59481\"],\"rets\":[0, 0]}";
    char *data = "{\"cid\":[\"b2836ba70f\"],\"rets\":[0]}";
    // uint32_t lenght = 0;
    int i = 0;
    uint32_t sum = 0;
...
void leshi_ctrl_switch_dev(void)
{
    ...
    /* 调光开关控制 */
    uint8_t data_on[] = {0x55, 0xaa, 0x00, 0x0c, 0x00, 0x10,
                        ...
                        0x0A, 0x30, 0x31, 0x63, 0x63, 0x62, 0x35, 0x63, 0x33, 0x30, 0x38,
                        0x14, 0x01, 0x00, 0x01, 0x01};
    // 18 03 00 0C
    // uint8_t data_on[] = {0x55, 0xaa, 0x00, 0x0c, 0x00, 0x20,
                        //  0x0A, 0x30, 0x38, 0x33, 0x31, 0x35, 0x31, 0x34, 0x39, 0x31, 0x39,
                        //  0x14, 0x01, 0x00, 0x01, 0x01,
                        //  0x18, 0x03, 0x00, 0x0c, 0x30, 0x30, 0x31, 0x33, 0x30, 0x32, 0x65, 0x65, 0x30, 0x32, 0x61, 0x32};

    // uint8_t data_on[] = {0x55, 0xAA, 0x00, 0x0C, 0x00, 0x1f 
    //         , 0x0A, 0x62, 0x30, 0x64, 0x31, 0x66, 0x35, 0x36, 0x35, 0x33, 0x36 
    //         , 0x69, 0x03, 0x00, 0x10, 0x33, 0x35, 0x30, 0x31, 0x66, 0x66, 0x66, 0x66, 0x66, 0x66, 0x66, 0x66, 0x66, 0x66, 0x30, 0x39
    // };
...
// 55 aa 00 14 00 0d 02 30 31 01 01 00 01 00 02 01 00 01 00 8a
void leshi_delete_one_dev(void)
{
    leshi_mesh_t test_cmd;
    // uint8_t *data = "{\"cid\":[\"b2838807f1\", \"b223f59481\"],\"rets\":[0, 0]}";
    char *data = "{\"sub_id\":\"b2836ba70f\",\"devkey\":\"01\"}"; /* ,\"devkey\":\"\" */
    // uint32_t lenght = 0;
    int i = 0;
    uint32_t sum = 0;
...
void leshi_group_ctrl_devs(void)
{
    ...
    /* 色温灯控制亮度 */
    // uint8_t data_on[] = {0x55, 0xaa, 0x00, 0x14, 0x00, 0x0B,
    //                     0x02, 0x30, 0x31,
    //                     0x16, 0x02, 0x00, 0x04, 0x00, 0x00, 0x03, 0xe8};
    /* 色温灯控制亮度 */
    uint8_t data_on[] = {0x55, 0xaa, 0x00, 0x14, 0x00, 0x08,
                        0x02, 0x30, 0x30,
                        0x14, 0x01, 0x00, 0x01, 0x01};
...
{
    // , 0x33, 0x38, 0x64, 0x34, 0x36, 0x36, 0x36, 0x38
    // 55 AA 00 0D 00 15 04 30 30 30 30 6B 03 00 0C 46 46 46 46 46 46 46 46 46 46 46 46 FC
    // 55 aa 00 0c 00 15 04 30 30 30 30 6b 03 00 0c 66 66 66 66 66 66 66 66 66 66 66 66 26

    uint32_t sum = 0;
    uint32_t i = 0;
    uint8_t data_on[] = {0x55, 0xAA, 0x00, 0x0c, 0x00, 0x15, 0x04, 0x30, 0x30, 0x30, 0x30, 0x6B, 0x03, 0x00, 0x0C, 0x66, 0x66, 0x66, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x66, 0x66};
    // uint8_t data_on[] = {0x55, 0xAA, 0x00, 0x0c, 0x00, 0x15, 0x04, 0x30, 0x30, 0x30, 0x30, 0x6B, 0x03, 0x00, 0x0C, 0x35, 0x35, 0x33, 0x38, 0x64, 0x34, 0x36, 0x36, 0x36, 0x38, 0x61, 0x61};
