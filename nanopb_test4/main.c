 ...
 * @LastEditors  : MCD
 * @LastEditTime : 2023-05-10 13:53:55
 * @FilePath     : /My_C_Test/nanopb_test4/main.c
 * @Description  : 
 * 
 * ******************************************
 */
...
static char *nano_pb_encode(size_t *size)
{
    ...
    com_msg.which_item = commonMessage_gw_info_tag;
    // com_msg.item.gw_info = calloc(1, sizeof(Gatewayinfo));
    ...
    memcpy(&com_msg.item.gw_info, &gw_info, sizeof(Gatewayinfo));
    ...
    com_msg.combo = calloc(com_msg.combo_size, sizeof(combineMsg *));
    for (i = 0; i < TEST_DEVICE_NEMBER; i++) {
        combineMsg *combo = calloc(1, sizeof(combineMsg));
        com_msg.combo[i] = combo;
        combo->index = i;
        combo->which_item = combineMsg_lsBTmeshGW_device_tag;
        struct _leshiBTmeshGWMsg *ls_msg = (struct _leshiBTmeshGWMsg *)calloc(1, sizeof(struct _leshiBTmeshGWMsg));
        combo->item.lsBTmeshGW_device = ls_msg;
        switch (i % 2) {
            case 0: {
                // printf("instream = %d\n", i);
                memcpy((char *)ls_msg->deviceId, "01d1dfb06a", sizeof("01d1dfb06a"));
                ls_msg->deviceName = (char *)calloc(32, sizeof(char));
                ...
                // ls_msg->dp.arg = ();
                ls_msg->dp_size = 4;
                ls_msg->dp = calloc(ls_msg->dp_size, sizeof(Datapoint *));

                ls_msg->dp[0] = calloc(1, sizeof(Datapoint));
                ls_msg->dp[0]->dpid = 1;
                ...
                ls_msg->dp[1] = calloc(1, sizeof(Datapoint));
                ls_msg->dp[1]->dpid = 1;
                ...
                ls_msg->dp[2] = calloc(1, sizeof(Datapoint));
                ls_msg->dp[2]->dpid = 1;
                ...
                ls_msg->dp[3] = calloc(1, sizeof(Datapoint));
                ls_msg->dp[3]->dpid = 1;
            ...
            case 1: {
                ...
                ls_msg->dp[0]->len = 5;
                ls_msg->dp[0]->which_t_value = Datapoint_v_raw_tag;
                memcpy((char *)ls_msg->dp[0]->t_value.v_raw, "abcde", sizeof("abcde"));
...
static void nano_pb_decode(char *buf, size_t size)
{
    ...
    // printf("decode status:%d type = %d, group id = %d\n", status, com_msg.type, com_msg.groupId);
    printf("gw_macaddr: %s\n", com_msg.item.gw_info.gw_macaddr);
    printf("v: %s\n", com_msg.item.gw_info.v);
    ...
    /* if(com_msg.combo)  */ {
        for (i = 0; i < com_msg.combo_size; i++) {
            if (com_msg.combo[i]->which_item == combineMsg_lsBTmeshGW_device_tag) {
                ...
                printf("dp_size: %d\n", com_msg.combo[i]->item.lsBTmeshGW_device->dp_size);
#if 1
                for (j = 0; j < com_msg.combo[i]->item.lsBTmeshGW_device->dp_size; j++) {
                    printf("dpid[%d]: %d\n", j, com_msg.combo[i]->item.lsBTmeshGW_device->dp[j]->dpid);
                    printf("type[%d]: %d\n", j, com_msg.combo[i]->item.lsBTmeshGW_device->dp[j]->type);
                    printf("len[%d]: %d\n", j, com_msg.combo[i]->item.lsBTmeshGW_device->dp[j]->len);
                    switch (com_msg.combo[i]->item.lsBTmeshGW_device->dp[j]->which_t_value) {
                        case Datapoint_v_raw_tag:
                            printf("v_raw[%d]: %s\n", j, com_msg.combo[i]->item.lsBTmeshGW_device->dp[j]->t_value.v_raw);
                            break;
                        case Datapoint_v_bool_tag:
                            printf("v_bool[%d]: %d\n", j, com_msg.combo[i]->item.lsBTmeshGW_device->dp[j]->t_value.v_bool);
                            break;
                        case Datapoint_v_value_tag:
                            printf("v_value[%d]: %d\n", j, com_msg.combo[i]->item.lsBTmeshGW_device->dp[j]->t_value.v_value);
                            break;
                        case Datapoint_v_string_tag:
                            printf("v_string[%d]: %s\n", j, com_msg.combo[i]->item.lsBTmeshGW_device->dp[j]->t_value.v_string);
                            break;
                        case Datapoint_v_enum_tag:
                            printf("v_enum[%d]: %d\n", j, com_msg.combo[i]->item.lsBTmeshGW_device->dp[j]->t_value.v_enum);
                            break;
                        case Datapoint_v_bitmap_tag:
                            printf("v_bitmap[%d]: %d\n", j, com_msg.combo[i]->item.lsBTmeshGW_device->dp[j]->t_value.v_bitmap);
                            break;
                    }
                }
#endif
...
int main(int argc, char const *argv[])
{
    ...
    printf("size = %ld\n", size);
    // sizeof(struct _leshiBTmeshGWMsg);
