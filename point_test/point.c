 ...
 * @LastEditors  : MCD
 * @LastEditTime : 2021-07-28 15:25:35
...
static device_t * _test_point(void)
{
    ...
    print_mcd("calloc size = %d, %d, %d", malloc_usable_size(dev_test), sizeof(device_t *), sizeof(device_info_t));
    // return 0;
    ...
    print_mcd("calloc size = %d, %d", malloc_usable_size(dev_test), malloc_usable_size(dev_info_test));
    if(dev_info_test == NULL)
        print_mcd("calloc failed !!");

    dev_test->device_info = dev_info_test;
    print_mcd("calloc size = %d, %d", malloc_usable_size(dev_test), malloc_usable_size(dev_test->device_info));
...
int main(int argc, char const *argv[])
{
    ...
    *pi = 314;
    ...
    char test[64] = {0};
    char test1[] = "this password";
    protolcol_t msg1;
    memset(&msg1, 0, sizeof(protolcol_t));
    memcpy(msg1.data, test1, sizeof(test1));
    print_mcd("%s", msg1.data);
    snprintf(test, strlen(msg1.data) + 1, "%s", msg1.data);
    // snprintf(test, MAXLEN * 2, "%s", test1);
    print_mcd("test = %s",test);
...
// 
    ...
    for (size_t i = 0; i < test_dev->num; i++)
    {
        ...
        print_mcd("%s, %s, %d, %d", test_dev->device_info[i].name, test_dev->device_info[i].deviceId,i, test_dev->device_info[i].online_state);   
    }
...
// 
    ...
    print_mcd("%d", malloc_usable_size(dev_test->device_info));
    free(dev_test);
    dev_test = NULL;
    print_mcd("%d", malloc_usable_size(dev_test));
    
    print_mcd("%d", malloc_usable_size(g_dev_test.device_info));
    ...
    for (size_t i = 0; i < g_dev_test.num; i++)
    {
        ...
        print_mcd("%s, %s, %d, %d", g_dev_test.device_info[i].name, g_dev_test.device_info[i].deviceId,i, g_dev_test.device_info[i].online_state);   
